...
The computed average recognition accuracy across all individuals was 91.14\%, indicating that the ResNet model generalized well across classes with minimal variance. This high accuracy demonstrates the model's robustness in distinguishing between visually similar faces with limited fine-tuning.

\begin{figure}[]
    \centering
        \includegraphics[width=.27\textwidth]{../blackbook/imgs/model_3_cp.jpg}
...
\begin{figure}
    \centering
    \includegraphics[width=.27\textwidth]{../blackbook/imgs/model_1_cm.jpg}
...
\begin{figure}[]
    \centering
    \includegraphics[width=.45\textwidth]{../blackbook/imgs/model_3_per_person.jpg}
    \caption{Accuracy per Person (Class) - Resnet Face Recognizer}
\end{figure}

\section{Results}
we evaluated three face recognition models—FaceNet, ResNet, and LBPH—based on Accuracy, Precision, Recall, and F1-Score. Each model was trained and tested on the same dataset under controlled conditions.
